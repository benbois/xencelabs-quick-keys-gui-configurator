...
void SetWindowIcon(GtkWindow *window, const guchar *buf, gsize len)
{
    ...
    if (gdk_pixbuf_loader_write(loader, buf, len, NULL) && gdk_pixbuf_loader_close(loader, NULL))
    {
        ...
        if (pixbuf)
        {
            ...
            /* Set default app icon (helps taskbar/dock) */
            gtk_window_set_default_icon(pixbuf);
            /* Provide multiple sizes for window managers that prefer specific dimensions */
            gint sizes[] = {16, 22, 32, 48, 64, 128, 256};
            GList *icon_list = NULL;
            for (guint i = 0; i < G_N_ELEMENTS(sizes); i++)
            {
                GdkPixbuf *scaled = gdk_pixbuf_scale_simple(pixbuf, sizes[i], sizes[i], GDK_INTERP_BILINEAR);
                if (scaled)
                {
                    icon_list = g_list_prepend(icon_list, scaled);
                }
            }
            if (icon_list)
            {
...
void SetDefaultAppIcon(const guchar *buf, gsize len)
{
    if (!buf || len == 0)
        return;
    GdkPixbufLoader *loader = gdk_pixbuf_loader_new();
...
static gboolean on_realize_set_wayland_app_id(GtkWidget *widget, gpointer data)
{
    const char *app_id = (const char *)data;
    GdkWindow *gdkwin = gtk_widget_get_window(widget);
    if (!gdkwin || !app_id || !app_id[0])
        return G_SOURCE_REMOVE;
    GdkDisplay *display = gdk_window_get_display(gdkwin);
    if (display && GDK_IS_WAYLAND_DISPLAY(display))
    {
        gdk_wayland_window_set_application_id(gdkwin, app_id);
    }
    return G_SOURCE_REMOVE;
}

void SetWaylandApplicationId(GtkWindow *window, const char *app_id)
{
    if (!app_id || !app_id[0])
        return;
    GtkWidget *widget = GTK_WIDGET(window);
    char *dup = g_strdup(app_id);
    g_object_set_data_full(G_OBJECT(window), "wayland-app-id", dup, (GDestroyNotify)g_free);
    ...
    {
        g_signal_connect(widget, "realize", G_CALLBACK(on_realize_set_wayland_app_id), dup);
    }
}
