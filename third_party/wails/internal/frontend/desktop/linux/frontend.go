...
func NewFrontend(ctx context.Context, appoptions *options.App, myLogger *logger.Logger, appBindings *binding.Bindings, dispatcher frontend.Dispatcher) *Frontend {
	...
	// Set default app icon before creating window (for taskbar, dock, etc.)
	if appoptions.Linux != nil && len(appoptions.Linux.Icon) > 0 {
		SetDefaultAppIcon(appoptions.Linux.Icon)
	}
