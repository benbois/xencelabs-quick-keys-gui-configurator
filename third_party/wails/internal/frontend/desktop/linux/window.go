...
/*
#cgo linux pkg-config: gtk+-3.0 gdk-wayland-3.0
...
func NewWindow(appoptions *options.App, debug bool, devtoolsEnabled bool) *Window {
	...
	if appoptions.Linux != nil {
		...
		if appoptions.Linux.ApplicationID != "" {
			appID := C.CString(appoptions.Linux.ApplicationID)
			defer C.free(unsafe.Pointer(appID))
			C.SetWaylandApplicationId(result.asGTKWindow(), appID)
		}
...
func (w *Window) Show() {
	if w.appoptions.StartHidden {
		w.SetSize(w.appoptions.Width, w.appoptions.Height)
	}
	// Re-apply icon when showing (some WMs need it after window is visible)
	if w.appoptions.Linux != nil && w.appoptions.Linux.Icon != nil {
		w.SetWindowIcon(w.appoptions.Linux.Icon)
	}
...
// SetDefaultAppIcon sets the default icon for all windows (taskbar, etc.). Call before creating windows.
func SetDefaultAppIcon(icon []byte) {
	if len(icon) == 0 {
		return
	}
	C.SetDefaultAppIcon((*C.guchar)(&icon[0]), (C.gsize)(len(icon)))
}
...
func (w *Window) Run(url string) {
	...
	if w.appoptions.StartHidden {
		C.gtk_widget_set_opacity(w.asGTKWidget(), C.gdouble(0))
		C.gtk_window_move(w.asGTKWindow(), C.gint(-10000), C.gint(-10000))
	}
	C.gtk_widget_show_all(w.asGTKWidget())
	if w.appoptions.StartHidden {
		w.Hide()
		C.gtk_widget_set_opacity(w.asGTKWidget(), C.gdouble(1))
	} else {
		w.Center()
	}
